...
typed AS (
    SELECT
        ...
        try_to_timestamp(regexp_replace(period.start, 'T', ' '), 'yyyy-MM-dd HH:mm:ssXXX') AS start_time,
        try_to_timestamp(regexp_replace(period.end, 'T', ' '), 'yyyy-MM-dd HH:mm:ssXXX') AS end_time,
        regexp_replace(serviceProvider.reference, '^(Organization/|urn:uuid:)', '') AS service_provider_id,
